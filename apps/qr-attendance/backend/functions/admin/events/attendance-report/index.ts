...
function rowType(t: unknown): string {
  if (t == null) return '';
  if (typeof Buffer !== 'undefined' && Buffer.isBuffer(t)) return t.toString('utf8').trim().toLowerCase();
...
function diffStayMinutes(inT: unknown, outT: unknown): number | null {
  if (isInvalidDatetime(inT) || isInvalidDatetime(outT)) return null;
...
function isEmptyOut(out: unknown): boolean {
  if (out == null || out === '') return true;
  if (out instanceof Date) return isInvalidDatetime(out);
 ...
 */
function mergeAttendanceSessions(rows: RawLogRow[]): Array<{
  log_id: number;
  email: string;
  user_name: string;
  in_time: string | null;
  ...
  while (i < sorted.length) {
    ...
    const stay =
      !isInvalidDatetime(inTimeRaw) && !isInvalidDatetime(outTimeRaw)
...
function computeStayStats(minutes: (number | null)[]): {
  avg_minutes: number | null;
  min_minutes: number | null;
  max_minutes: number | null;
} {
  const vals = minutes.filter((m): m is number => m != null && Number.isFinite(m) && m >= 0);
  if (vals.length === 0) {
...
export const handler = async (
  ...
  try {
    ...
    const data = await withConnection(pool, async (conn) => {
      ...
      const mergedLogs = mergeAttendanceSessions(rawRows as RawLogRow[]);
    ...
    const slotRows = data.timeSlotStats as any[];
    ...
    return successResponse({
      ...
      statistics: {
        time_slot_distribution: slotRows.map((stat: any) => ({
